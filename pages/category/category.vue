<template>
	<view class="category-list">
		...
		<scroll-view class="left" scroll-y>
			<view class="row" v-for="(item, index) in categoryList" :key="item.cid" :class="{ row_active: showCategoryIndex === index }" @click="handleLeftRowClick(index)">
...
export default {
	data() {
		...
		onLoad() {
				this.$request("/api/super_classify/apikey/maxd").then(res => {
					if (res.code === 1) {

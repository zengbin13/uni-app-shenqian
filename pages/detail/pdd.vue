	...
	<view class="goods-detail" v-if="goodsItem.img">
		...
		<view class="goods-info" v-if="goods.shopname">
			...
			<view class="goods-price">
				...
				<view class="goods-price-right">{{ goods.itemsale }}人付款</view>
			</view>
		</view>
		<!-- 优惠券 -->
...
export default {
	...
	methods: {
		...
		addBrowseHistory() {
			...
			let flag = pddBrowseList.find(item => item.itemid === this.goodsItem.itemid);
			if (!flag) {
		...
		// 加载商品详情数据
		getGoodsInfo() {
			let url = `/tao/pdd/details/${this.id}`;
			this.$request(url).then(res => {
				
				let resData = res.data.goodsDetailResponse.goodsDetails[0];
				// 轮播图数据
				if (resData.goodsGalleryUrls) {
					this.swiperList = resData.goodsGalleryUrls
				} else {
					this.swiperList.push(resData.goodsImageUrl);
				}
				// 数据修正
